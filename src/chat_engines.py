...
def get_condense_question_chat_engine(llm, memory=None):
...
def get_context_chat_engine(llm, retriever=None,memory=None,prefix_messages=None):
    memory = Memory.from_defaults(token_limit=500)
    embed_model = HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")
    # Empty Index with embedding model specified
    index = VectorStoreIndex.from_documents([], embed_model=embed_model)
...
def get_condense_plus_context_chat_engine(llm, retriever=None,memory=None,prefix_messages=None):
    memory = Memory.from_defaults(token_limit=500)
    embed_model = HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")
    # Empty Index with embedding model specified
    index = VectorStoreIndex.from_documents([], embed_model=embed_model)
